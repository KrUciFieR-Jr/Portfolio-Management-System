...
public class FrameController implements IFrameController {
  ...
  DocumentListener portfolioListener = new DocumentListener() {
    ...
    private void checkPortfolio() {
      ...
      view.setErrorLabels("portfolio", isValidPortfolioName ? "" : "Invalid Portfolio Name");
      view.setErrorLabels("numStocks", isValidNumberOfStocks ? "" : "Invalid Stocks Number");
  ...
  DocumentListener stockListener = new DocumentListener() {
    ...
    private void checkStockDetails() {
      ...
      view.setErrorLabels("quantity", isValidQuantityOfStocks ? "" : "Invalid Stock Quantity");
  ...
  DocumentListener weightageListener = new DocumentListener() {
    ...
    private void checkData() {
      ...
      view.setErrorLabels("stockName", isValidStockName ? "" : "Invalid Stock Name");
      view.setErrorLabels("weightageOfStocks", isValidQuantityOfStocks ? "" : "Invalid Weightage");
  ...
  DocumentListener updateCommissionListener = new DocumentListener() {
    ...
    private void checkCommissionListener() {
      ...
      view.setErrorLabels("commissionText", isValidCommission ? "" : "Invalid Commission");
  ...
  DocumentListener executeStrategyDateListener = new DocumentListener() {
    ...
    private void checkStrategyParameters() {
      ...
      view.setErrorLabels("numStocks", isValidStockQuantity ? "" : "Invalid Stock Numbers");
  ...
  private void configureButtonListener() {
    ...
    this.buttonClickedMap.put("Submit Portfolio", () -> {
      ...
      if (fromStrategy) {
        this.executeStrategy(model.createFlexiblePortfolio(portfolioName), maxStocks, false);
      } else {
    ...
    this.buttonClickedMap.put("Submit Commission", () -> {
      ...
      try {
        ...
        this.view.showDialogBox("Commission Updated Successfully", "Successful", false);
        this.mainMenu();
    ...
    this.buttonClickedMap.put("Submit on Date Strategy", () -> {
      ...
      if (data.get("date").equals("")) {
        this.view.showDialogBox("The Date Entered is Invalid", "Invalid Date", true);
        return;
      }
    ...
    this.buttonClickedMap.put("Submit on Date Range", () -> {
      ...
      if (data.get("date").equals("")) {
        this.view.showDialogBox("The Start Date Entered is Invalid", "Invalid Date", true);
        return;
      }
      if (data.get("anotherDate").equals("")) {
        this.view.showDialogBox("The End Date Entered is Invalid", "Invalid Date", true);
        return;
      }
      date1 = LocalDate.parse(data.get("date"));
    ...
    this.buttonClickedMap.put("Submit Weightage", () -> {
      ...
      stocksDetails = model.getPortfolioComposition(portfolioNumber, LocalDate.now());
      stockName = stocksDetails.get(numStock).split("\\|")[1];
      this.getGetStrategyWeightage(numStock + 1, maxStocks, portfolioNumber, isFromDateRange
              ? "" : stockName);
    ...
    this.buttonClickedMap.put("Submit Portfolio Choice", () -> {
      ...
      switch (method) {
        case "composition":
          String[] columnHeaders = {"id", "Stock Code",
                                    "Name of Company","Number of Shares Brought"};
          try {
